...
final class MakeCommandCommand extends Command
{
    ...
    public function handle(
        ...
        if ($name === '' || !preg_match('/^[A-Z][A-Za-z0-9]+$/', $name)) {
            $name = text(
                label: 'Command name',
                default: $name !== '' ? $name : 'RenameDocument',
                validate: function (string $v) {
                    return preg_match('/^[A-Z][A-Za-z0-9]+$/', $v)
                        ? null
                        : 'Please use PascalCase (e.g. RenameDocument).';
                },
                hint: 'Example: RenameDocument, PublishInvoice'
            );
            if ($name === '') {
                $this->error('Aborted.');
                return self::FAILURE;
        ...
        if ($style === PathStyle::Subcontext->value && ($subcontext === null || $subcontext === '')) {
            $subcontext = (string) text(
                label: 'Subcontext folder (e.g. Writer)',
        ...
        // --- Compute placement -------------------------------------------------
        $placement = $resolver->resolve($registry, 'command', [
        ...
        // --- Plan + write files -----------------------------------------------
        $plan = $scaffolder->planCommand($name, $placement);
        ...
        $this->info('Command created and registered.');
        return self::SUCCESS;
    }
}
